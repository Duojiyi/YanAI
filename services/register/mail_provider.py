...
class MoeMailProvider(BaseMailProvider):
    ...
    def __init__(self, entry: dict, conf: dict):
        ...
        self.api_key = str(entry["api_key"]).strip()
        self.default_domain = str(entry.get("default_domain") or "moemail.app").strip()
        self.domain = [str(item).strip() for item in (entry.get("domain") or []) if str(item).strip()]
    ...
    def create_mailbox(self, username: str | None = None) -> dict[str, Any]:
        ...
        domain = _next_domain(self.domain) if self.domain else self.default_domain
        if domain:
            payload["domain"] = domain
        data = self._request("POST", "/api/emails/generate", payload=payload)
        ...
        if not address or not email_id:
            raise RuntimeError("MoeMail 缺少 email/address 或 id")
        return {"provider": self.name, "provider_ref": self.provider_ref, "address": address, "email_id": email_id}

    def fetch_latest_message(self, mailbox: dict[str, Any]) -> dict[str, Any] | None:
        email_id = str(mailbox.get("email_id") or mailbox.get("id") or "").strip()
        if not email_id:
            raise RuntimeError("MoeMail mailbox 缺少 email_id")
        data = self._request("GET", f"/api/emails/{email_id}")
        items = data.get("messages") if isinstance(data, dict) else data
        messages = [item for item in items if isinstance(item, dict)] if isinstance(items, list) else []
        if not messages:
            return None
        item = max(messages, key=lambda value: ((_parse_received_at(value.get("received_at") or value.get("receivedAt") or value.get("date") or value.get("created_at")) or datetime.fromtimestamp(0, tz=timezone.utc)).timestamp(), str(value.get("id") or "")))
        message_id = str(item.get("id") or item.get("messageId") or "").strip()
        if message_id:
            detail = self._request("GET", f"/api/emails/{email_id}/{message_id}")
            if isinstance(detail, dict):
                item = detail.get("message") if isinstance(detail.get("message"), dict) else detail
        text_content, html_content = _extract_content({
            **item,
            "text_content": item.get("content") or item.get("text_content") or item.get("text"),
            "html_content": item.get("html") or item.get("html_content"),
        })
        return {
            "provider": self.name,
            ...
            "sender": str(item.get("from_address") or item.get("fromAddress") or item.get("from") or ""),
            "text_content": text_content,
            "html_content": html_content,
            "received_at": _parse_received_at(item.get("received_at") or item.get("receivedAt") or item.get("date") or item.get("created_at")),
            "raw": item,
        }
